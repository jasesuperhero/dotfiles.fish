...
# Resilient: installs each missing plugin individually and NEVER aborts the
# bootstrap run if one can't be fetched (network / removed ref) — it warns and
# continues. Already-installed plugins are skipped so re-runs are offline-safe.
...
# Plugin fetch issues must not fail the whole bootstrap run.
exit 0
