...
function M.detectors.lsp(buf)
  ...
  for _, client in pairs(User.lsp.get_clients { bufnr = buf }) do
...
function M.info()
  ...
  lines[#lines + 1] = "```"
  User.info(lines, { title = "User Roots" })
...
function M.setup()
  vim.api.nvim_create_user_command(
    "LazyRoot",
    function() User.root.info() end,
    { desc = "User roots for the current buffer" }
  )
...
function M.get(opts)
  ...
  return User.is_win() and ret:gsub("/", "\\") or ret
end
